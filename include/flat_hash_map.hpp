...
#ifdef _MSC_VER
#define NOINLINE(...) __declspec(noinline) __VA_ARGS__
#else
#define NOINLINE(...) __VA_ARGS__ __attribute__((noinline))
#endif
...
namespace pp {
    ...
    template<typename T>
    struct flat_hash_entry {
        flat_hash_entry() {}
        flat_hash_entry(int8_t distance_from_desired)
        : distance_from_desired(distance_from_desired) {
        }

        static flat_hash_entry *empty_default_table() {
            static flat_hash_entry result[min_lookups] = {{},
                                                          {},
                                                          {},
                                                          {special_end_value}};
            return result;
    ...
    class flat_hash_table : private EntryAlloc, private Hasher, private Equal {
        using Entry = flat_hash_entry<T>;
    ...
    public:
        ...
        explicit flat_hash_table(size_type bucket_count, const ArgumentHash &hash = ArgumentHash(),
        ...
        flat_hash_table(size_type bucket_count, const ArgumentAlloc &alloc)
        : flat_hash_table(bucket_count, ArgumentHash(), ArgumentEqual(), alloc) {
        }

        flat_hash_table(size_type bucket_count, const ArgumentHash &hash, const ArgumentAlloc &alloc)
        : flat_hash_table(bucket_count, hash, ArgumentEqual(), alloc) {
        }

        explicit flat_hash_table(const ArgumentAlloc &alloc)
        : EntryAlloc(alloc) {
        }

        template<typename It>
        flat_hash_table(It first, It last, size_type bucket_count = 0, const ArgumentHash &hash = ArgumentHash(),
                        const ArgumentEqual &equal = ArgumentEqual(),
                        const ArgumentAlloc &alloc = ArgumentAlloc())
                        : flat_hash_table(bucket_count, hash, equal, alloc) {
            insert(first, last);
        }

        template<typename It>
        flat_hash_table(It first, It last, size_type bucket_count, const ArgumentAlloc &alloc)
        : flat_hash_table(first, last, bucket_count, ArgumentHash(), ArgumentEqual(), alloc) {
        }

        template<typename It>
        flat_hash_table(It first, It last, size_type bucket_count, const ArgumentHash &hash,
                        const ArgumentAlloc &alloc)
                        : flat_hash_table(first, last, bucket_count, hash, ArgumentEqual(), alloc) {
        }

        flat_hash_table(std::initializer_list <T> il, size_type bucket_count = 0,
        ...
        flat_hash_table(std::initializer_list <T> il, size_type bucket_count, const ArgumentAlloc &alloc)
        : flat_hash_table(il, bucket_count, ArgumentHash(), ArgumentEqual(), alloc) {
        }

        flat_hash_table(std::initializer_list <T> il, size_type bucket_count, const ArgumentHash &hash,
                        const ArgumentAlloc &alloc)
                        : flat_hash_table(il, bucket_count, hash, ArgumentEqual(), alloc) {
        }

        flat_hash_table(const flat_hash_table &other)
        : flat_hash_table(other,
                          AllocatorTraits::select_on_container_copy_construction(other.get_allocator())) {
        }

        flat_hash_table(const flat_hash_table &other, const ArgumentAlloc &alloc)
        ...
        flat_hash_table(flat_hash_table &&other) noexcept
        : EntryAlloc(std::move(other)), Hasher(std::move(other)), Equal(std::move(other)) {
            swap_pointers(other);
        }

        flat_hash_table(flat_hash_table &&other, const ArgumentAlloc &alloc) noexcept
        ...
        const_iterator find(const FindKey &key) const {
            return const_cast<flat_hash_table *>(this)->find(key);
        }
        ...
        void swap(flat_hash_table &other) {
            using std::swap;
            swap_pointers(other);
    ...
    private:
        ...
        void rehash_for_other_container(const flat_hash_table &other) {
            rehash(std::min(num_buckets_for_reserve(other.size()), other.bucket_count()));
        }

        void swap_pointers(flat_hash_table &other) {
            using std::swap;
        ...
        template<typename Key, typename... Args>
        NOINLINE(std::pair < iterator, bool >)
    ...
    class flat_hash_map: public flat_hash_table <
            std::pair < K, V>,
            K,
            H,
            ...
            >,
            E,
            KeyOrValueEquality<K, std::pair < K, V>, E>,
            A,
            typename std::allocator_traits<A>::template rebind_alloc<flat_hash_entry<std::pair < K, V>>>
            > {
        using Table = flat_hash_table <
                std::pair < K, V>,
                K,
                H,
                KeyOrValueHasher<K, std::pair < K, V>, H>,
